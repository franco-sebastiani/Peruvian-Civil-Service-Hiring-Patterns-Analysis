...
def main():
    ...
    if col1.button("💾 Save", type="primary", use_container_width=True):
        ...
        general_kw_list = [k.strip() for k in general_keywords.split(',') if k.strip()] if general_keywords else []
        specific_kw_list = [k.strip() for k in specific_keywords.split(',') if k.strip()] if specific_keywords else []
        
        # Save to sample (handle None and empty strings)
        samples[idx]['experience_general_description'] = general_desc.strip() if general_desc and general_desc.strip() else None
        samples[idx]['experience_general_keywords'] = general_kw_list if general_kw_list else []
        samples[idx]['experience_specific_description'] = specific_desc.strip() if specific_desc and specific_desc.strip() else None
